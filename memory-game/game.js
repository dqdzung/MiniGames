...
class MemoryGame extends Phaser.Scene {
	create() {
		...
		this.add.text(MARGIN, 18 * S, "Memory Match", {
			fontSize: px(24),
			color: "#fff",
			fontStyle: "bold",
			padding: { y: 6 },
		...
		this.movesText = this.add
			.text(GAME_W - MARGIN, 24 * S, "Moves: 0", {
				fontSize: px(18),
				color: "#aab",
				padding: { y: 6 },
			})
	...
	makeCard(x, y, symbol) {
		...
		const back = this.add
			.rectangle(0, 0, CARD_W, CARD_H, 0x4e54c8)
		...
		const face = this.add
			.rectangle(0, 0, CARD_W, CARD_H, 0xf5f5fa)
			.setStrokeStyle(2 * S, 0xcfcfe6);
		const emoji = this.add
			.text(0, 0, symbol, { fontSize: px(48), padding: { y: 6 } })
			.setOrigin(0.5);
	...
	win() {
		...
		this.add
			.text(GAME_W / 2, GAME_H / 2 - 20 * S, "You win! 🎉", {
				fontSize: px(36),
				color: "#fff",
				fontStyle: "bold",
				padding: { y: 8 },
		...
		this.add
			.text(
				GAME_W / 2,
				GAME_H / 2 + 24 * S,
				`${this.moves} moves — click to replay`,
				{
					fontSize: px(18),
					color: "#aab",
					padding: { y: 6 },
