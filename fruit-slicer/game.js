...
class FruitSlicerGame extends Phaser.Scene {
	create() {
		this.score = 0;
		this.lives = LIVES;
		this.over = false;
		...
		this.add
			.text(GAME_W / 2, GAME_H - 22 * S, "Swipe to slice — avoid 💣", {
				fontSize: px(13),
				color: "#889",
				padding: { y: 4 },
			})
	...
	spawnObject() {
		...
		this.objects.push({
			...
			sprite: this.add
				.text(x, GAME_H + FRUIT_R, isBomb ? BOMB : fruit.emoji, {
					fontSize: px(46),
					padding: { y: 6 },
				})
				.setOrigin(0.5),
	...
	splatter(o) {
		for (let k = 0; k < 8; k++) {
			...
			this.tweens.add({
				targets: dot,
				x: o.x + Math.cos(ang) * sp,
				y: o.y + Math.sin(ang) * sp,
				alpha: 0,
				scale: 0.2,
				duration: 420,
				ease: "Quad.easeOut",
	...
	update(time, delta) {
		...
		for (let i = this.objects.length - 1; i >= 0; i--) {
			...
			if (o.x < o.r) {
				o.x = o.r;
				o.vx = Math.abs(o.vx);
			} else if (o.x > GAME_W - o.r) {
				o.x = GAME_W - o.r;
				o.vx = -Math.abs(o.vx);
			}
			o.sprite.setPosition(o.x, o.y);
			o.sprite.rotation += o.spin * dt;
			if (o.y > GAME_H + o.r * 2 && o.vy > 0) {
				// fell back off the bottom
