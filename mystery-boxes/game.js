...
class MysteryBoxGame extends Phaser.Scene {
	create() {
		...
		this.add
			.text(GAME_W / 2, 26 * S, "Pick a Gift Box 🎁", {
				fontSize: px(26),
				color: "#fff",
				fontStyle: "bold",
				padding: { y: 6 },
			...
			.setOrigin(0.5);
		this.hint = this.add
			.text(GAME_W / 2, 52 * S, "Choose one — good luck!", {
				fontSize: px(15),
				color: "#aab",
				padding: { y: 6 },
			})
	...
	makeBox(x, y, content) {
		...
		const rect = this.add
			.rectangle(0, 0, BOX, BOX, 0x4e54c8)
			.setStrokeStyle(3 * S, 0x8a8fff);
		const front = this.add
			.text(0, 0, "🎁", { fontSize: px(50), padding: { y: 6 } })
			.setOrigin(0.5);
		const emoji = this.add
			.text(0, -18 * S, content.emoji, { fontSize: px(40), padding: { y: 6 } })
			.setOrigin(0.5)
			.setVisible(false);
		const label = this.add
			.text(0, 28 * S, content.label, {
				fontSize: px(13),
				color: "#fff",
				align: "center",
				fontStyle: "bold",
	...
	onPick(chosen) {
		...
		this.time.delayedCall(700, () => {
			this.boxes.forEach((b) => {
				if (b !== chosen) this.reveal(b, false);
			});
	...
	showResult(content) {
		...
		this.hint
			.setText(msg)
			.setColor(content.win ? "#ffd166" : "#aab")
			.setFontSize(px(18));
