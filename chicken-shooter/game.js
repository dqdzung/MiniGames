...
class ChickenShooterGame extends Phaser.Scene {
  create() {
    ...
    this.add.rectangle(0, GAME_H * BUSH_Y, GAME_W, GAME_H * (1 - BUSH_Y), 0x4a7a3a).setOrigin(0, 0).setDepth(3);
    this.bushX = [];
    for (let i = 0; i < 5; i++) {
  ...
  drawBarn(cx, groundY) {
    ...
    const w = 130 * S, h = 78 * S;
    const left = cx - w / 2, top = groundY - h;
    g.fillStyle(0x5c3a2e, 1); // roof
    g.fillPoints([{ x: left - 8 * S, y: top }, { x: cx, y: top - 46 * S }, { x: left + w + 8 * S, y: top }], true);
    ...
    g.beginPath();
    g.moveTo(left - 8 * S, top); g.lineTo(cx, top - 46 * S); g.lineTo(left + w + 8 * S, top);
    g.strokePath();
  ...
  drawWindmill(cx, groundY) {
    const g = this.add.graphics().setDepth(1);
    const h = 150 * S, topW = 26 * S, botW = 48 * S;
    ...
    const hubY = topY - 6 * S, L = 60 * S, bw = 12 * S;
  ...
  drawBush(cx, baseY, w) {
    ...
    const dark = 0x2f5326, mid = 0x3c6630, light = 0x5a9a44;
    const r = w * 0.3;
    const blob = (dx, dy, rr, c) => { g.fillStyle(c, 1); g.fillCircle(cx + dx, baseY + dy, rr); };
    g.fillStyle(dark, 1); g.fillEllipse(cx, baseY + r * 0.35, w * 1.02, r * 1.1); // base shadow on the ground
  ...
  pts(base, scale, mult) {
    return Phaser.Math.Clamp(Math.round((base * mult / scale) / 5) * 5, MIN_PTS, MAX_PTS);
  }
  ...
  spawnFlyer() {
    ...
    const scale = close ? Phaser.Math.FloatBetween(1.7, 2.4) : Phaser.Math.FloatBetween(0.7, 1.3);
    const points = this.pts(FLYER_BASE, scale, FLYER_MULT);
    const baseY = close ? Phaser.Math.Between(GAME_H * 0.55, GAME_H * 0.72)
  ...
  spawnPop() {
    ...
    this.tweens.add({ targets: sprite, scaleX: sx, scaleY: scale, duration: 130, ease: "Back.easeOut" });

    const linger = close ? Phaser.Math.Between(1800, 2800) : Phaser.Math.Between(1200, 2200);
  ...
  popOut(ch) {
    ...
    this.tweens.add({
      targets: ch.sprite, scaleX: 0, scaleY: 0, duration: 130, ease: "Back.easeIn",
  ...
  feathers(x, y) {
    for (let k = 0; k < 7; k++) {
      const f = this.add.circle(x, y, Phaser.Math.Between(3, 7) * S, 0xfff4d6).setDepth(9);
  ...
  update(time, delta) {
    ...
    for (let i = this.chickens.length - 1; i >= 0; i--) {
      ...
      if ((ch.vx > 0 && ch.sprite.x > GAME_W + 64 * S) ||
  ...
  gameOver() {
    ...
    this.chickens.forEach((ch) => { if (ch.timer) ch.timer.remove(); ch.sprite.destroy(); });
    this.chickens = [];
