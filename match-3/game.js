...
class Match3Game extends Phaser.Scene {
	create() {
		...
		this.scoreText = this.add
			.text(GAME_W - MARGIN, 26 * S, "Score: 0", {
				fontSize: px(20),
				color: "#ffd166",
				fontStyle: "bold",
				padding: { y: 6 },
		...
		this.timerText = this.add
			.text(GAME_W / 2, 26 * S, this.fmtTime(this.timeLeft), {
				fontSize: px(20),
				color: "#fff",
				fontStyle: "bold",
				padding: { y: 6 },
		...
		// board background
		this.add
			.rectangle(MARGIN, TOP + MARGIN, COLS * CELL, ROWS * CELL, 0x0f1830)
			.setOrigin(0, 0)
	...
	drawGem(g, base, cx, cy, R) {
		...
		const T1 = P(-0.55, -0.62),
			T2 = P(0.55, -0.62); // table (top) corners
		const M1 = P(-1, -0.05),
			M2 = P(1, -0.05); // girdle (widest) points
		const IL = P(-0.45, -0.05),
			IR = P(0.45, -0.05); // inner table-base points
		const B = P(0, 1); // pavilion tip

		g.fillStyle(lighter);
		g.fillPoints([T1, T2, IR, IL], true); // table
		g.fillStyle(light);
		g.fillPoints([T1, IL, M1], true); // crown left (lit)
		g.fillStyle(dark);
		g.fillPoints([T2, M2, IR], true); // crown right (shade)
		g.fillStyle(base);
		g.fillPoints([IL, IR, B], true); // pavilion center
		g.fillStyle(light);
		g.fillPoints([M1, IL, B], true); // pavilion left
		g.fillStyle(dark);
		g.fillPoints([IR, M2, B], true); // pavilion right
	...
	onPick(p) {
		...
		const adj =
			Math.abs(this.selected.r - r) + Math.abs(this.selected.c - c) === 1;
		if (adj) {
	...
	swapModel(a, b) {
		...
		[this.sprites[a.r][a.c], this.sprites[b.r][b.c]] = [
			this.sprites[b.r][b.c],
			this.sprites[a.r][a.c],
		];
	}
	...
	gameOver(msg) {
		...
		this.add
			.text(GAME_W / 2, GAME_H / 2 - 24 * S, msg, {
				fontSize: px(30),
				color: "#fff",
				fontStyle: "bold",
				align: "center",
				wordWrap: { width: GAME_W - 40 * S },
				padding: { y: 8 },
			})
			.setOrigin(0.5);
		this.add
