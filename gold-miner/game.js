...
// buried loot: value, weight (heavier = slower reel-in), scale (size), spawn (rarity weight)
const LOOT = [
  { emoji: "💰", value: 50,  weight: 0.8, scale: 0.8, spawn: 20 }, // gold nugget
  { emoji: "💰", value: 100, weight: 1.0, scale: 1.2, spawn: 22 }, // gold
  { emoji: "💰", value: 150, weight: 1.7, scale: 1.6, spawn: 10 }, // big gold
  { emoji: "💰", value: 200, weight: 2.6, scale: 2.0, spawn: 4 },  // gold boulder (heavy, rich)
  { emoji: "💎", value: 300, weight: 0.6, scale: 0.9, spawn: 2 },  // diamond (rarest, top value, fast)
  { emoji: "🪨", value: 5,   weight: 1.3, scale: 1.0, spawn: 14 }, // pebble
  { emoji: "🪨", value: 10,  weight: 2.3, scale: 1.4, spawn: 14 }, // rock
  { emoji: "🪨", value: 15,  weight: 2.8, scale: 1.8, spawn: 9 },  // big rock
  { emoji: "🪨", value: 20,  weight: 3.4, scale: 2.2, spawn: 6 },  // boulder (very heavy, cheap)
...
class GoldMinerGame extends Phaser.Scene {
  create() {
    ...
    this.add.rectangle(0, DIRT_Y, GAME_W, GAME_H - DIRT_Y, 0x6b4f38).setOrigin(0, 0);
    this.add.rectangle(0, DIRT_Y, GAME_W, 4 * S, 0x8a6a4a).setOrigin(0, 0); // dirt surface line
    this.add.text(PIVOT.x, 58 * S, "👷", { fontSize: px(34), padding: { y: 6 } }).setOrigin(0.5).setDepth(6);
    ...
    this.timerText = this.add.text(GAME_W - 16 * S, 12 * S, this.fmtTime(this.timeLeft), {
      fontSize: px(20), color: "#fff", fontStyle: "bold", padding: { y: 6 },
    }).setOrigin(1, 0).setDepth(10);
    this.add.text(GAME_W / 2, GAME_H - 20 * S, "Click / tap to drop the claw", {
      fontSize: px(13), color: "#cbbfa8", padding: { y: 4 },
  ...
  spawnItem() {
    ...
    const sprite = this.add.text(x, y, type.emoji, { fontSize: px(34 * type.scale), padding: { y: 6 } })
      .setOrigin(0.5).setDepth(2);
    this.items.push({ sprite, x, y, r, value: type.value, weight: type.weight, bomb: type.bomb });
  }

  hookTip() {
    return { x: PIVOT.x + Math.sin(this.theta) * this.len, y: PIVOT.y + Math.cos(this.theta) * this.len };
  }

  // the actual grabbing point: the claw mouth, a bit below the rope ring
  clawPoint() {
    const tip = this.hookTip();
    return { x: tip.x + Math.sin(this.theta) * CLAW_REACH, y: tip.y + Math.cos(this.theta) * CLAW_REACH };
  }
  ...
  drawHook(tip) {
    ...
    const ux = Math.sin(this.theta), uy = Math.cos(this.theta);
    const vx = Math.cos(this.theta), vy = -Math.sin(this.theta);
    const w = (pv, pu) => ({ x: tip.x + vx * pv * S + ux * pu * S, y: tip.y + vy * pv * S + uy * pu * S });
    ...
    const prong = (sgn) => g.strokePoints([
      w(0, 6), w(sgn * 2.5, 9), w(sgn * 6, 12), w(sgn * 9, 15), w(sgn * 9, 18), w(sgn * 5, 20),
    ], false);
    prong(1); prong(-1);
  }
  ...
  update(time, delta) {
    ...
    if (this.state === "swing") {
      ...
      for (let i = this.items.length - 1; i >= 0; i--) {
        ...
        if (Phaser.Math.Distance.Between(claw.x, claw.y, it.x, it.y) < HOOK_R + it.r) {
      ...
      if (this.state === "extend" &&
          (this.len >= MAX_LEN || claw.y >= GAME_H - 8 * S || claw.x <= 8 * S || claw.x >= GAME_W - 8 * S)) {
        this.state = "retract";
      }
    } else if (this.state === "retract") {
      const sp = this.carry ? RETRACT_SPEED / this.carry.weight : RETRACT_SPEED * 1.8;
      this.len -= sp * dt;
      if (this.carry) {
        const claw = this.clawPoint();
  ...
  detonate(bomb) {
    const bx = bomb.sprite.x, by = bomb.sprite.y, AOE = 130 * S;
    bomb.sprite.destroy();
  ...
  explosion(x, y, radius) {
    const blast = this.add.circle(x, y, radius, 0xff7b00, 0.45).setDepth(7).setScale(0.2);
    this.tweens.add({ targets: blast, scale: 1, alpha: 0, duration: 350, onComplete: () => blast.destroy() });
    const boom = this.add.text(x, y, "💥", { fontSize: px(50), padding: { y: 6 } }).setOrigin(0.5).setDepth(8).setScale(0.5);
    this.tweens.add({ targets: boom, scale: 1.4, alpha: 0, duration: 420, onComplete: () => boom.destroy() });
  }

  scorePopup(x, y, pts) {
    const t = this.add.text(x, y, "+" + pts, {
      fontSize: px(18), color: "#ffe14d", fontStyle: "bold", padding: { y: 4 },
    }).setOrigin(0.5).setDepth(11);
    this.tweens.add({ targets: t, y: y - 40 * S, alpha: 0, duration: 700, onComplete: () => t.destroy() });
  }
  ...
  gameOver() {
    ...
    this.add.rectangle(GAME_W / 2, GAME_H / 2, GAME_W, GAME_H, 0x000000, 0.74).setDepth(200);
